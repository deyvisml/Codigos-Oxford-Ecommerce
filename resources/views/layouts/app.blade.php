...
<body class="">
    ...
    <div class="bg-sky-900">
        <div class="max-w-7xl mx-auto px-2">
            <nav class="flex flex-wrap items-center justify-between  py-2 w-full  sm:gap-x-4 gap-x-2">
                <div class="flex flex-wrap items-center justify-center w-full md:justify-between md:w-2/12">
                    <a href="/" class="w-auto" title="Inicio">
                        <img src="{{ asset('images/logo.png') }}" alt="imagen logo" title="logo códigos oxford"
                            class="object-contain max-h-[75px]">
                    </a>
                </div>
                ...
                <form action="{{ route('search.index') }}" method="get"
                    ...
                    <div class="flex items-center w-full overflow-hidden rounded md:w-full border-y border-s ">
                        <input type="text" name="keyword" placeholder="Buscar por titulo, ISBN, etc."
                            class="w-11/12 p-2 outline-none" required>
                            ...
                            <li>
                                <a class="block w-full px-4 py-2 text-sm font-normal bg-transparent whitespace-nowrap text-neutral-700 hover:bg-neutral-100 active:text-neutral-800 active:no-underline disabled:pointer-events-none disabled:bg-transparent disabled:text-neutral-400 dark:text-neutral-200 dark:hover:bg-neutral-600"
                                    ...
                                    <img src="{{ asset('images/country-flags/' . $country->flag) }}"
                                        class="inline-block w-6 h-6 mb-1 ms-2" alt="{{ $country->flag }}"
                                        title="{{ $country->flag }}">
                                </a>
                            </li>
                        @endforeach
                ...
                <div class="w-auto min-w-[150px] my-2 flex justify-end">
                        ...
                        <div class="relative" data-te-dropdown-ref>
                            ...
                            <a class="flex items-center justify-center transition duration-150 ease-in-out cursor-pointer hidden-arrow whitespace-nowrap motion-reduce:transition-none"
                                ...
                                <img src="{{ auth()->user()->picture }}"
                                    class="border-2 rounded-full w-9 h-9 border-neutral-300 me-2" alt="user image"
                                    title="user" loading="lazy" />
                                <p class="text-sm cursor-pointer text-neutral-100 me-1">{{ auth()->user()->first_name }}
                                </p>

                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mt-1 text-neutral-100">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    @endauth
                    @guest
                        <a href="/google-auth/redirect" title="ingresar" class="text-left text-gray-100">Ingresar /
                            Registrarse</a>
                    @endguest
                </div>
            </nav>
    ...
    <div class=" border-gray-300  bg-neutral-50 border-y-4">
        <div class="max-w-7xl mx-auto px-0">
            <nav>
                <ul class="flex flex-wrap justify-start text-sm font-semibold text-slate-600 ">
                    @foreach ($categories as $category)
                        <li
                            ...
                            <a href="{{ route('series.index', ['category' => $category]) }}"
                                title="{{ $category->name }}"
                                class="block py-1.5 px-4  hover:text-slate-900 hover:bg-neutral-200">
                                {{ $category->name }}
                            </a>
                        </li>
                    @endforeach
    ...
    <footer class="flex items-center justify-center h-24 bg-sky-200">
        <p class="text-center">Códigos Oxford 2013-2025 ©</p>
    </footer>

    <div class="z-20 bottom-5 fixed right-5">
        <div class="flex flex-col gap-y-2">
            <a href="https://www.facebook.com/codigos.ingles" target="_blank">
                <img class="w-12" src="{{ asset('images/social-media/facebook.png') }}" alt="icono whatsapp"
            ...
            <a href="https://api.whatsapp.com/send?phone=51938544411&amp;text=Hola, vengo de la página web y necesito mas información sobre el producto"
                target="_blank">
                <img class="w-12" src="{{ asset('images/social-media/whatsapp.png') }}" alt="icono whatsapp"
                    title="icono whatsapp">
            </a>
        </div>
    </div>
